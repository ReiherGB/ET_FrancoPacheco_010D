productos = {'8475HD': ['HP', 15.6, '8GB', 'DD', '1T', 'Intel Core i5', 'Nvidia GTX1050'],
             '2175HD': ['lenovo', 14, '4GB', 'SSD', '512GB', 'Intel Core i5', 'Nvidia GTX1050'],
             'JjfFHD': ['Asus', 14, '16GB', 'SSD', '256GB', 'Intel Core i7', 'Nvidia RTX2080Ti'],
             'fgdxFHD': ['HP', 15.6, '8GB', 'DD', '1T', 'Intel Core i3', 'integrada'],
             'GF75HD': ['Asus', 15.6, '8GB', 'DD', '1T', 'Intel Core i7', 'Nvidia GTX1050'],
             '123FHD': ['lenovo', 14, '6GB', 'DD', '1T', 'AMD Ryzen 5', 'integrada'],
             '342FHD': ['lenovo', 15.6, '8GB', 'DD', '1T', 'AMD Ryzen 7', 'Nvidia GTX1050'],
             'UWU131HD': ['Dell', 15.6, '8GB', 'DD', '1T', 'AMD Ryzen 3', 'Nvidia GTX1050']}

stock = {'8475HD': [387990,10], '2175HD': [327990,4], 'JjfFHD': [424990,1],
        'fgdxFHD': [664990,21], '123FHD': [290890,32], '342FHD': [444990,7],
        'GF75HD': [749990,2], 'UWU131HD': [349990,1], 'FS1230HD': [249990,0]}



def stock_marca(marca):
    stock_total = 0
    for modelo, datos in productos.items():
        if stock[modelo][1] != 0 and datos [0].lower() == marca:
            stock_total += stock[modelo][1]
    print(f"El stock disponible es: {stock_total}")

def busqueda_precio(p_min, p_max):
    modelo_en_pantalla = []
    for modelo,(precio,stock) in stock.items():
        if p_min <= precio <= p_max:
            if stock[modelo][1] != 0:
                modelo_en_pantalla.append(f"{productos,[modelo][0]}--{modelo}")
    if modelo_en_pantalla:
        print(f"Los notebooks entre los precios que consultas son: {sorted(modelo_en_pantalla)}")
    else:
        print("No hay notebooks en ese rango de precios")

def actualizar_precio(modelo, p):
     if modelo in stock:
        stock[modelo][1] = p
        return True
     else:
         return False
def Salir():
    print("Programa finalizado")


while True:

    print("\n***MENU PRINCIPAL***")
    print("1. Stock marca")
    print("2. Busqueda por precio")
    print("3. Actualizar precio")
    print("4. Salir")
    opcion = input("Ingrese la opcion que desea: ")
    match opcion:

        case "1":
            marca = input("Ingrese marca a consultar: ").lower()

        case "2":
            p_min = int(input("Ingrese precio minimo:"))
            p_max =int(input("Ingrese precio maximo: "))
        case "3":
            while True:
                modelo = input("Ingrese el modelo a actualizar: ")
                try:
                    p = int(input("Ingrese precio nuevo: "))
                except ValueError:
                    print("Debe ingresar valores enteros!!!")

